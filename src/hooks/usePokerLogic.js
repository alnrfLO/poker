...
export function usePokerLogic({ gameState, updateState, nextPhase }) {
...
const handleAction = useCallback(async (action, raiseAmount = 0) => {
  ...
  let { players, pot, currentBet, currentPlayerIndex, communityCards, actedIds = [] } = gameState;
  players = [...players.map((p) => ({ ...p }))];
  const player = players[currentPlayerIndex];
  
  let newActedIds = [...actedIds];
  if (!newActedIds.includes(player.id)) newActedIds.push(player.id);
  
  if (action === ACTIONS.FOLD) {
    players[currentPlayerIndex].status = PLAYER_STATUS.FOLDED;
    const active = players.filter((p) => p.status === PLAYER_STATUS.ACTIVE);
    if (active.length === 1) {
      await updateState({ players, actedIds: newActedIds });
    ...
    pot += total;
    // Reset car relance — tout le monde doit rejouer sauf celui qui a relancé
    newActedIds = [player.id];
  }
  
  const activePlayers = players.filter((p) => p.status === PLAYER_STATUS.ACTIVE);
  const allActed = activePlayers.every((p) => newActedIds.includes(p.id));
  const allCalled = activePlayers.every((p) => p.bet === currentBet);
  
  if (allActed && allCalled) {
    await updateState({ players, pot, currentBet, actedIds: [] });
